...
**Concern:** Governance, security, and organisational oversight. Ensures the platform is configured correctly, controls who has access, and maintains visibility across all projects. Acts as the ultimate authority.

- Designated list of admins (configured in the system)
- Configures global platform settings (region, model access, policies)
- Creates projects
- Assigns one or more Cost Centre Owners to each project
- Can approve/reject any key request (bypassing the Cost Centre Owner)
- Views usage dashboards and cost reports across all projects
...
- Can hold **multiple keys simultaneously** (e.g., keys for different projects)
- Requests a key by selecting a **project** from the list of available projects
- Views their own active keys, usage, and limit status
- Copies key + setup instructions to configure Claude Code locally
- Can **self-deactivate** any of their own keys (e.g., when leaving a project)
- Can see when they've been stopped (limit reached) and when tokens become available again

### 2.3 Cost Centre Owner

**Concern:** Managing the cost and usage of Claude Code within their project(s). They own the budget and need to ensure developers are using it responsibly. They decide who gets access and what limits apply.

A project can have **multiple Cost Centre Owners** (e.g., a project lead and a finance lead both managing the same project).

- Created/assigned by an Administrator for one or more projects
- Does not necessarily need a Bedrock key themselves (but can also be a Developer)
- **Approves or rejects** developer key requests for their project(s)
...
- Can revoke keys for developers in their project
- Manages the overall project budget (total cap across all keys in the project)
- Has a dashboard scoped to their project(s)
- Can view:
  - Which developers are using keys in their project
...
- **Configures alerts** (email and Slack) for their project:
  - Budget threshold alerts (e.g., 50%, 80%, 100% of project budget used)
...
- **Proof of Concept**: Hard-coded usernames and passwords for simplicity
  - e.g., `admin/admin`, `dev1/dev1`, `projectadmin1/projectadmin1`
  - No external auth dependencies for the PoC

#### 3.1.2 Key Request Flow
1. Developer clicks "Request Key"
2. Developer selects a **project** (from list of existing projects)
3. Developer optionally enters a justification/description
4. Request enters **Pending Approval** state
5. The **Cost Centre Owner** for that project is notified
6. Cost Centre Owner reviews and approves/rejects the request
7. On approval, the system **automatically** provisions the key, tagged with the project code for cost attribution
...
- Usage tracked and attributed to the project
- Project-level budget applies across all keys in the project
...
- View all pending key requests across all projects
- Approve/reject any key request (bypasses Cost Centre Owner)
- Bulk approve/reject

#### 3.2.2 Project Management
- Create new projects
- Assign/remove one or more Cost Centre Owners to/from each project
- Deactivate projects (revokes all associated keys)
...
- Search/filter by developer, project code, status

#### 3.2.4 Cost & Usage Dashboard
- View aggregate usage and cost
- Break down costs by project code
...
- View pending key requests for their project(s)
...
- Revoke keys for developers in their project

#### 3.3.2 Project Usage View (scoped to their project(s) only)
...
- **Cost tracking**: estimated cost per developer, total project cost, cost over time
- **Charts & Visualisations**:
  - Daily/weekly/monthly usage trends (line charts)
  - Cost burn-down / burn-rate chart
  - Model mix pie chart
  - Top consumers bar chart
- **Budget management**: set project budget cap, view remaining budget, percentage used
...
- Customisable thresholds:
  - Project budget: notify at X% used (e.g., 50%, 80%, 100%)
...
- Tag all resources with metadata (project code, developer, created date, expiry)
...
- Track usage per key, per developer, per project
- Provide cost estimates based on token consumption and model pricing
- **Hard budget limits**: when a project reaches its budget cap, all keys for that project are automatically **stopped** (not slowed — fully blocked until sufficient usage falls outside the rolling window)
...
- Content filtering per project or globally
...
- Keys shown to the developer only once at creation (or retrievable via secure mechanism)
- Audit log of all key operations (create, approve, revoke, use)
...
2. Selects project (e.g., `PRJ-1234`) and provides optional description
3. Sees "Pending Approval" status
4. Cost Centre Owner reviews and approves (sets per-key constraints)
5. Developer is notified: "Your key for PRJ-1234 is ready"
6. Developer copies setup instructions, pastes into terminal, runs `claude` — it works immediately
7. Usage is tracked against `PRJ-1234`; Cost Centre Owner can view dashboards
8. If developer finishes on the project, they can self-deactivate the key
...
4. Activate cost tracking mechanisms for project-level attribution
...
| 2 | Approvers | Cost Centre Owners approve for their projects; Admins can bypass |
...
| 6 | Project code validation | Free-text for now; future integration possible |
...
| 14 | Multiple keys per developer | Yes — can hold multiple project keys simultaneously |
| 15 | Multiple Cost Centre Owners | Yes — a project can have multiple owners |
