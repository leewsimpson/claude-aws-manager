...
**Phase 7 complete** — the ⭐ **clickable local prototype milestone** is reached: log in → request key → approve with constraints (key lands in `ready`) → **developer retrieves the token once** from their dashboard → key active → mock usage accrues → spend tracks rolling/lifetime limits and CC budget → key **hard-stops** when a limit is crossed (and restarts when the rolling window clears). See `docs/implementation-log.md` for detailed progress and `docs/implementation-plan.md` for what's next (**Phase 8 — dashboards & visualisations**). Monorepo: `/backend` (FastAPI), `/frontend` (React+Vite), `/docker`, root `docker-compose.yml`. DB at Alembic head `d4e5f6a7b8c9` (`keys.token_retrieved_at` + nullable `credential_id` + `ready` status); the only remaining deferred tables are `alert_*` → P9.

**Deferred credential / token retrieval:** approval **never reveals the bearer token** — separation of duties between the approver (CCO/Admin) and the credential holder (developer). `provision_for_request` creates only the IAM identity (`aws.create_key_identity`) and lands the key in **`ready`** (`credential_id = NULL`). The developer claims it via `POST /keys/{id}/retrieve` (owner or admin only) → `aws.issue_credential` mints the token, returned **once**, key → `active`, `token_retrieved_at` stamped. Auto-approval (CCO requesting for self) also lands `ready` and is retrieved like any developer. In-app notification = a banner + per-key "Retrieve token" action on the Keys page (email/Slack stays deferred). The mock splits identity vs. credential (`_identities` keyed by `iam_username`); `rehydrate_aws_from_db` re-registers `ready` identities so retrieval survives a restart. Audit: `key.provisioned` (status `ready`, no credential), `key.token_retrieved`.
...
- **Bearer tokens are NEVER stored.** The credential is issued — and the token returned **once** — only when the **developer** retrieves the key (`POST /keys/{id}/retrieve`) or regenerates it; **approval never reveals it** (the approver isn't the holder). Lost token → regenerate. Do not add token persistence.
